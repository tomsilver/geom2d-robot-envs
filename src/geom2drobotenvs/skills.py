...
def _iter_motion_plans_to_rectangle(
    ...
    for approach_theta, target_size in [
        ...
        pose_plan = _get_motion_plan_to_rectangle_side(
            approach_theta,
            target_size,
            state,
            robot,
...
def _get_motion_plan_to_rectangle_side(
    ...
    # Run motion planning.
    return run_motion_planning_for_crv_robot(
        state,
        robot,
        target_pose,
        action_space,
...
def create_rectangle_vacuum_table_place_inside_option(
    ...
    name = "RectangleVacuumPlaceInside"
...
def create_rectangle_vacuum_table_place_on_option(
    ...
    name = "RectangleVacuumPlaceOn"
    # robot, held object, target surface
    params_space = ObjectSequenceSpace([CRVRobotType, RectangleType, RectangleType])
    assert isinstance(action_space, CRVRobotActionSpace)
    ...
    def _initiable(
        ...
        robot, held_obj, surface = params

        static_object_body_cache: dict[Object, MultiBody2D] = {}
        suctioned_objs = get_suctioned_objects(state, robot)

        # Try to approach the surface from above. After, try to extend the arm
        # as far as possible until a collision is about to occur.
        robot_base_radius = state.get(robot, "base_radius")
        held_obj_thickness = state.get(held_obj, "height")
        pad = 1.25 * robot_base_radius
        robot_to_target_side_dist = robot_base_radius + held_obj_thickness + pad

        target_rect = rectangle_object_to_geom(state, surface, static_object_body_cache)
        target_height = target_rect.height

        # Try approaching the rectangle from each of four sides, while at the
        # farthest possible distance.
        approach_theta = -np.pi / 2
